---
title: "Voeding en management voor hoge productie "
...
#| results: 'asis'
#| warning: false
tabr <- read.xlsx("beelden/20251103_rantsoen_dalfsen.xlsx",
                   sheet = "Blad1",
                   colNames = TRUE)

tabr |> gt(id="two",rowname_col = "X1") |> 
  tab_header(title = "Analyse van rantsoen VOF van Dalfsen-Kronenberg") |> 
  fmt_number(
    columns = c(Per.Koe),
    decimals = 2,
    use_seps = FALSE
  ...
  fmt_number(
    columns = c(DS,VEM,RE,DVE,OEB,Zetmeel),
    decimals = 0,
    use_seps = FALSE
  ) |> 
  sub_missing(
...
![Bomb-calorimeter](beelden/bomb-calorimeter.png)
...
![Fistuleerde koe](beelden/fistuleerde_koe.jpg)
...
tab1 <- data.frame(Behandeling = c("Standaard gemalen",
  ...
  tab_footnote("@nrc2021") |> 
  fmt_markdown(columns = Behandeling)
tab1
```
...
tab <- data.frame(c(Groei = "23.5 MJ/kg LW",
...
![](beelden/larsen2014.png){width="100%"}
